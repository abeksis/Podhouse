...
**Reviewed by:** Adam Twilley ([JAVE-Gateway](https://github.com/JAVE-Gateway))
**Reviewed version:** Podhouse 0.9.0
**Published against:** 0.10.3

This is his audit, published with a line under each entry saying what this
project did about it: fixed and in which release, still open, or disputed and
why. Nothing has been removed because it was uncomfortable — the entries he
withdrew and the ones I argued with are all here.

It is the reviewer's redacted version. The detailed one names files, line
numbers and reproduction steps, and a public document like that about issues
that are still open is a map for somebody attacking other people's boxes, not
only mine. Anyone who wants the detail can reproduce it from the source, which
is all here.

Eleven entries were fixed and released in 0.10.0 through 0.10.3, each one
reproduced on a throwaway box before and after the change. The rest are named
as open, including the two that are real work rather than patches: signing
releases, and splitting the dashboard so the web process holds no Docker
socket.
...
> **Open.** Releases are not signed and the updater does not verify a signer. This is real work rather than a patch, and it is named as open in docs/SECURITY.md.
...
> **Open.** Same as #2: the tarball path has no trusted verification. The default path is a git clone of a tag.
...
> **Open.** The install Worker serves what is on main over HTTPS and nothing more. It is a transport, not an authenticity guarantee, and is documented as such.
...
> **Partly fixed in 0.10.0.** Write requests now require a header token that a cross-site request cannot attach, which covers the forged-request case. An explicit Origin check is still worth adding.
...
> **Fixed in 0.10.0.** The throttle counts the connection address, and only credits a forwarded one when the connection itself came from this box's own networks.
...
> **Fixed in 0.10.0, and 0.10.3 for existing files.** Rollback archives, staging files and decrypted restores are created private, in private directories, before any secret is written. A failed authentication deletes its plaintext. Archives written by older versions are closed by the 0.10.3 migration.
...
> **Fixed in 0.10.0.** The cookie alone now authorises reads only. Everything that changes the box also requires a token returned once at login and held in this origin's localStorage, which an app on another port cannot read.
...
> **Fixed in 0.10.0.** SVG from a URL is refused, downloaded bytes are identified by signature rather than by Content-Type, and user content is served with a policy that allows it nothing.
...
> **Fixed in 0.10.0.** The request target is parsed inside the error boundary and against a fixed base; a malformed Host answers 400 and the process stays up.
...
> **Fixed in 0.10.0.** Auth mutations are serialised, so a login cannot write back a password that has just been changed. Proven with a test that forces the interleaving. Cross-process writes (CLI and dashboard together) are still open and are listed as such in docs/SECURITY.md.
...
> **Fixed in 0.10.0.** The page is served with a content security policy, nosniff, a same-origin referrer policy and framing denied.
...
> **Open.** One operation per module at a time is enforced; there is no global bound on concurrent privileged work.
...
> **Open.** The restore path still passes its key as an argument. Worth moving to an environment variable or a pipe.
...
> **Fixed in 0.10.0.** Icon URLs resolving to loopback, link-local, private ranges or LAN-only names are refused.
...
> **Open.** Setup scripts still receive their secrets as arguments.
...
> **Fixed in 0.10.0.** Passing the limit now ends the request instead of rejecting a promise while the body keeps growing.
...
> **Fixed in 0.10.0.** Login is serialised with the rest of the auth mutations, so attempts cannot pass each other's accounting.
...
> **Fixed in 0.10.0.** One policy file (dashboard/lib/policy.js) defines the rule, and the CLI validator, the CLI install path, the dashboard install path and the App Store generator all use it. Values are checked, not just key names.
...
> **Fixed in 0.10.0, corrected in 0.10.3.** The check stops rather than continuing when it cannot read the release control file. 0.10.1 made that true and thereby refused every update on a box that had updated before; 0.10.3 reads the file two ways and stops only when both fail.
...
> **Answered.** The dashboard holds a read-write Docker socket and that is root on the box. It is the first line of docs/SECURITY.md, docs/DOCKER-SOCKET.md lists every call it makes, and splitting the dashboard so the web process holds no socket is the next piece of work. Portainer stopped being installed by default in 0.9.0, and the metrics agent lost the socket in 0.8.1.
...
> **Withdrawn by the reviewer.** The backup download was not shown to be exfiltrable by a cross-site request.
...
> **Open.** The CLI evaluates fixed expression strings written in this repository, not user input. Replacing them with a plain accessor is worth doing and changes behaviour, so it is a refactor rather than a patch.
...
> **Open.** Setup scripts receive the whole of .env rather than the values they declare.
...
> **Not a finding.** Expired sessions are pruned when a session is created.
...
> **Not a finding.** The filename is validated before it reaches a header.
...
> **Open.** The .env replacement helper interpolates values into a pattern. Today it is only handed version strings and generated keys; it is not safe for arbitrary text and should be rewritten before it ever is.
...
> **Open.** A tree already owned by root from an old install needs a deliberate recovery step rather than a silent fallback.
...
> **Not a finding.** The comment matches the implementation.
...
> **Fixed in 0.7.0 onward.** Every module now declares what it may do, and the two that genuinely need the host say privileged: true with the reason next to it. docs/MODULE-SCHEMA.md describes the rule.
...
> **By design.** `homebox secrets <module>` prints that module's secrets to somebody who already has a root shell on the box.
...
> **Accepted.** Inline scripts are passed fixed source and data through arguments. Extracting them helps testing and is not a security change.
...
> **Open.** Metadata read failures are swallowed, which can hide a broken module rather than report it.
...
> **By design.** The container list is a host-admin view; it is behind the session like everything else.
...
> **Open.** Error text shown to a signed-in administrator can include internal paths. Worth trimming.
...
> **Accepted constraint.** LAN-only is a deployment assumption, not something the process enforces. Said plainly in docs/SECURITY.md.
...
> **Open.** An event stream already open outlives the session that started it until it reconnects.
...
> **Open.** A backup that was written is not proof that a restore of it produces a working box. Testing restores is the only answer and is not automated.
